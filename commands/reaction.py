...
def _has_send(channel: Optional[object]) -> bool:
...
def setup(handler: CommandHandler) -> None:
    ...
    async def create_reaction_role(ctx: SlashContext, role: Role):
        ...
        if not _has_send(channel):
            ...
            return
        sendable_channel = cast(SendableChannel, channel)
        ...
        channel_mention = getattr(sendable_channel, "mention", f"<#{stored_channel_id}>")
        await ctx.send(
