"""Ban log channel configuration command."""
...
def setup(handler: CommandHandler) -> None:
    ...
    store = resources.ban_log_store
    logger = resources.logger

    @slash_command(name="banlog", description="Set the channel for ban logs.")
    @slash_option(
        name="channel",
        description="Channel to post ban logs in (defaults to the current channel).",
        required=False,
        opt_type=OptionType.CHANNEL,
    )
    @slash_default_member_permission(Permissions.MANAGE_GUILD)
    async def set_ban_log_channel(ctx: SlashContext, channel=None):
        if not ctx.guild_id:
        ...
        if not target_channel or not getattr(target_channel, "send", None):
            ...
            if channel_id:
                try:
                    ...
                    logger.error("Unable to fetch channel %s for ban log: %s", channel_id, error)
                    target_channel = None
        ...
        await ctx.send(f"Ban logs will be sent to {channel_mention}.", ephemeral=True)

    handler.register_slash_command(set_ban_log_channel)
