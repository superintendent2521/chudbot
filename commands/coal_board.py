"""Coal board configuration command."""
...
def setup(handler: CommandHandler) -> None:
    ...
    store = resources.coal_board_store
    logger = resources.logger

    @slash_command(name="coalboard", description="Set the channel for coal board posts.")
    @slash_option(
        name="channel",
        description="Channel to post coal board messages in (defaults to the current channel).",
        required=False,
        opt_type=OptionType.CHANNEL,
    )
    @slash_default_member_permission(Permissions.MANAGE_GUILD)
    async def set_coal_board_channel(ctx: SlashContext, channel=None):
        if not ctx.guild_id:
        ...
        if not target_channel or not getattr(target_channel, "send", None):
            ...
            if channel_id:
                try:
                    ...
                    logger.error("Unable to fetch channel %s for coal board: %s", channel_id, error)
                    target_channel = None
        ...
        await ctx.send(f"Coal board posts will be sent to {channel_mention}.", ephemeral=True)

    handler.register_slash_command(set_coal_board_channel)
