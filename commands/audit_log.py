"""Audit log configuration command."""
...
def setup(handler: CommandHandler) -> None:
    ...
    store = resources.audit_log_store
    logger = resources.logger

    @slash_command(name="auditlog", description="Set the channel for deleted-message audit logs.")
    @slash_option(
        name="channel",
        description="Channel to post audit log messages in (defaults to the current channel).",
        required=False,
        opt_type=OptionType.CHANNEL,
    )
    @slash_default_member_permission(Permissions.MANAGE_GUILD)
    async def set_audit_log_channel(ctx: SlashContext, channel=None):
        if not ctx.guild_id:
        ...
        if not target_channel or not getattr(target_channel, "send", None):
            ...
            if channel_id:
                try:
                    ...
                    logger.error("Unable to fetch channel %s for audit log: %s", channel_id, error)
                    target_channel = None
        ...
        await ctx.send(f"Deleted-message audit logs will be sent to {channel_mention}.", ephemeral=True)

    handler.register_slash_command(set_audit_log_channel)
